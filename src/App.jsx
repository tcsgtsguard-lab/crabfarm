...
function MarketPricesPanel({ marketPrices, settings, onAdd, onDelete }) {
  ...
  const [myTavilyKey, setMyTavilyKey] = useState(() => {
    try {
      return localStorage.getItem("cf_tavily_api_key") || "";
    } catch {
      return "";
    }
  });
  const [showTavilyKeyBox, setShowTavilyKeyBox] = useState(false);
  const saveTavilyKey = (val) => {
    setMyTavilyKey(val);
    try {
      if (val) localStorage.setItem("cf_tavily_api_key", val);
      else localStorage.removeItem("cf_tavily_api_key");
    } catch {}
  };
  ...
  const searchAi = async () => {
    if (!myTavilyKey) {
      setAiError("\u0E01\u0E23\u0E38\u0E13\u0E32\u0E43\u0E2A\u0E48 Tavily API Key \u0E01\u0E48\u0E2D\u0E19 (\u0E01\u0E14 \u2699\uFE0F \u0E14\u0E49\u0E32\u0E19\u0E25\u0E48\u0E32\u0E07) \u0E2A\u0E21\u0E31\u0E04\u0E23\u0E1F\u0E23\u0E35\u0E44\u0E14\u0E49\u0E17\u0E35\u0E48 tavily.com");
      setShowTavilyKeyBox(true);
      return;
    }
    setAiLoading(true);
    setAiError("");
    ...
    try {
      ...
      const res = await fetch("/api/market-price-search", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ locationName, tavilyApiKeyOverride: myTavilyKey || void 0, geminiApiKeyOverride: myApiKey || void 0 })
      });
